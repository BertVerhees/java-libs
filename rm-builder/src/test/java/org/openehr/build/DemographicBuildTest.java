...
public class DemographicBuildTest extends BuildTestBase {
	...
	public void testBuildContact() throws Exception {
		...
		addresses.add(new Address(null, node, text("address"), null, null,
				null, null, itemSingle("address detail")));
		values.put("name", text);
		values.put("archetypeNodeId", node);
		...
		assertEquals("archetypeNodeId wrong", node, contact
				.getArchetypeNodeId());
		...
		contact = (Contact) obj;
		assertEquals("timeValidity wrong", timeValidity, contact
				.getTimeValidity());
	}
	...
	public void testBuildPartyRelationship() throws Exception {
		...
		DvInterval<DvDate> timeValidity = new DvInterval<DvDate>(new DvDate(
				"2001-10-30"), null);
		ObjectRef source = new ObjectRef(hid("1.8.3.4.3.6.1"), "LOCAL", "PARTY");
		ObjectRef target = new ObjectRef(hid("1.2.13.3.7.31.1"), "LOCAL",
				"PARTY");
		...
		assertEquals("timeValidity wrong", timeValidity, pi.getTimeValidity());
	...
	public void testBuildRole() throws Exception {
		...
		identities.add(new PartyIdentity(null, node,
				...
				itemSingle("legal name")));
		Archetyped archetypeDetails = new Archetyped(new ArchetypeID(
				"openehr-dm_rm-Role.doctor.v2"), "v1.0");
		PartyRef performer = new PartyRef(new HierObjectID("1.2.4.5.6.12.1"),
				"PERSON");
		...
		assertEquals("archetypeDetails wrong", archetypeDetails, role
				.getArchetypeDetails());
	...
	private void verifyBuildActor(String subclass) throws Exception {
		...
		identities.add(new PartyIdentity(null, "at0003",
				...
				itemSingle("legal name")));
		Archetyped archetypeDetails = new Archetyped(new ArchetypeID(
				"openehr-dm_rm-Actor.actor.v2"), "v1.0");
		values.put("uid", uid);
		values.put("name", text);
		values.put("archetypeNodeId", node);
		values.put("details", item);
		...
		assertTrue("subclass wrong", actor.getClass().getName().endsWith(
				subclass));
	}
	 ...
	 * 
	 * The Initial Developer of the Original Code is Rong Chen. Portions created
	 * by the Initial Developer are Copyright (C) 2003-2008 the Initial
