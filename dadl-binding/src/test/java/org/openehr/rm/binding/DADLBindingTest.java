...
public class DADLBindingTest extends DADLBindingTestBase {
	...
	public void testBindTypedTextElement() throws Exception {
		...
		assertTrue("rmObject not Element", rmObj instanceof Element);		
		Element element = (Element) rmObj;
		assertEquals("element name wrong", "text", 
				element.getName().getValue());
		assertEquals("element archetypeNodeId wrong", "at0001", 
				element.getArchetypeNodeId());
	...
	public void testUntypedDvCodedText() throws Exception {
		...
		assertNotNull("rmObject null", rmObj);
		assertTrue("rmObject not DvCodedText: "	+ rmObj.getClass().getName(), 
	...
	public void testBindSimpleTypedItemTree() throws Exception {
		...
		assertNotNull("rmObject null", rmObj);
		assertTrue("rmObject not ItemTree: "	+ rmObj.getClass().getName(), 
				rmObj instanceof ItemTree);
		
		ItemTree itemTree = (ItemTree) rmObj;
		assertEquals("ItemList name wrong", "item tree", 
				itemTree.getName().getValue());
		assertEquals("itemTree.items size wrong", 2, itemTree.getItems().size());
		assertEquals("itemTree.items[0].value.magnitude wrong", 120.0,
				((DvQuantity) ((Element) itemTree.getItems().get(0)).getValue()).getMagnitude(), 0);
		assertEquals("itemTree.items[1].value.magnitude wrong", 80.0,
	...
	public void testBindComplexItemTree() throws Exception {
		...
		assertEquals("ItemList name wrong", "item tree", 
				itemTree.getName().getValue());
		assertEquals("itemTree.items size wrong", 3, itemTree.getItems().size());
		assertEquals("itemTree.items[0].value.magnitude wrong", 120.0,
				((DvQuantity) ((Element) itemTree.getItems().get(0)).getValue()).getMagnitude(), 0);
		assertEquals("itemTree.items[1].value.magnitude wrong", 80.0,
				((DvQuantity) ((Element) itemTree.getItems().get(1)).getValue()).getMagnitude(), 0);
		
		Object value = itemTree.itemAtPath("/items[at0006]/items[at0007]/value");
		assertTrue(value instanceof DvQuantity);
		assertEquals(150.0, ((DvQuantity) value).getMagnitude(), 0);
