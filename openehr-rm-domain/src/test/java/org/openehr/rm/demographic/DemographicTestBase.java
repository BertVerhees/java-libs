...
public class DemographicTestBase  {
	...
	protected DvCodedText coded(String value, String code) {
		return new DvCodedText(value, lang(), encoding(), new CodePhrase(
				SNOMED_CT, code), ts);
	}
	...
	protected PartyRef partyRef(String id, String type) {
		return new PartyRef(oid(id), type);
	}

	protected ObjectRef contriRef(String id) {
		return new ObjectRef(oid(id), "LOCAL", "CONTRIBUTION");
	}
	...
	protected EventContext eventContext() throws Exception {
		...
		Participation part1 = new Participation(provider("1.3.24.2.3.4.2",
				"ORGANISATION"), coded("participation function", "23432423"),
		...
		return new EventContext(provider("1.2.3.4.2.5", "ORGANISATION"),
				datetime("2000-10-10T10:00:00"), null, participations,
				"event context location", coded("event context setting",
						"2342342"), itemSingle("other context"), ts);
	}
	...
	protected Instruction instruction() throws Exception {
		...
		Archetyped archetypeDetails = new Archetyped(new ArchetypeID(
				"openehr-ehr_rm-instruction2.diabetes.v1"), "1.2");
		...
		String[] types = { "PARTY" };
		List<Participation> participations = participationList(
				"1.2.3.4.5.6.11", 1, types);
		...
		return new Instruction(uid, meaning, name, archetypeDetails,
				feederAudit, links, null, language("en"), language("en"),
				subject("1.4.55.1.4.2.4"), provider("1.2.15.2.5.15.4",
						"ORGANISATION"), null, participations, protocol,
				guidelineId, text("narrative"), null, null, null, ts);
	}
	...
	protected History<ItemSingle> event(String name) {
		...
		return new History<ItemSingle>(null, "at0002", text("history"), null,
				null, null, null, new DvDateTime("2006-06-25T23:11:11"), items,
				DvDuration.getInstance("PT1h"), DvDuration.getInstance("PT3h"),
				null);
	}

	protected PartySelf subject(String id) {
		return new PartySelf(partyRef(id, "PERSON"));
	}

	protected ObjectRef guideline(String id) {
		return new ObjectRef(oid(id), "LOCAL", "GUIDELINE");
	}

	protected List<Participation> participationList(String id, int num,
	...
	protected Participation participation(String id, String type)
			throws Exception {
		return new Participation(provider(id, type), coded(
				"participation function", id + 1), coded("participation mode ",
				id + 1), new DvInterval<DvDateTime>(
				datetime("2000-10-10 10:00:00"),
				datetime("2001-10-10 10:00:00")), ts);
	}

	protected PartyIdentified provider(String id, String type) throws Exception {
		PartyRef performer = new PartyRef(new HierObjectID(id), type);
